...
def test_to_binary_kitchen():
    """Test Nexus -> Binary: Kitchen et al. 2009 format"""
    ...
    assert re.search(r"1\s+ALL_0,", nexus)
    assert re.search(r"2\s+ALL_1,", nexus)
    assert re.search(r"3\s+ASHES_0,", nexus)
    assert re.search(r"4\s+ASHES_1,", nexus)
    assert re.search(r"5\s+ASHES_2", nexus)
